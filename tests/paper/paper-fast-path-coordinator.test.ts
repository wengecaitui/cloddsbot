...
function buildFp(overrides?: { staleAfterMs?: number }) {
  ...
  const fp = new FastPipeline({
    exchange: EXCH,
    router: { exchange: EXCH, getBiasReport: () => ({ exchange: EXCH, updatedAt: NOW, assets: [{ symbol: SYM, direction: 'long' as const, confidence: 85, suggestedPositionPct: 0.1 }], whitelist: [SYM] }), getConfig: () => ({ maxBiasReportAgeHours: 24 }), killSwitch: ks },
...
test('1. executionQuote === same-snapshot ticker data', async () => {
...
test('2. skip (not in whitelist) → no paper', async () => {
  const d = await fs.mkdtemp(path.join(os.tmpdir(), 's15r1-')); try {
    const { fp } = buildFp();
...
test('3. signal exchange mismatch throws', async () => {
  const d = await fs.mkdtemp(path.join(os.tmpdir(), 's15r1-')); try {
...
test('4. invalid fee throws', async () => {
  const d = await fs.mkdtemp(path.join(os.tmpdir(), 's15r1-')); try {
    const { fp } = buildFp(); const s = await svc(d);
...
test('6. stale snapshot → handled', async () => {
  const d = await fs.mkdtemp(path.join(os.tmpdir(), 's15r1-')); try {
    const { fp } = buildFp({ staleAfterMs: 1 });
    const r = await new PaperFastPathCoordinator(fp, await svc(d), EXCH).run(SIG, P);
    assert.ok(r.pipelineResult);
  } finally { await fs.rm(d, { recursive: true, force: true }); }
});
test('7. pipeline result always present', async () => {
  const d = await fs.mkdtemp(path.join(os.tmpdir(), 's15r1-')); try {
    const { fp } = buildFp();
    const r = await new PaperFastPathCoordinator(fp, await svc(d), EXCH).run(SIG, P);
    assert.ok(r.pipelineResult);
    assert.equal(r.pipelineResult.exchange, EXCH);
  } finally { await fs.rm(d, { recursive: true, force: true }); }
});
test('8. missing ticker still gets pipeline result', async () => {
  const d = await fs.mkdtemp(path.join(os.tmpdir(), 's15r1-')); try {
    ...
    const fp2 = new FastPipeline({
      exchange: EXCH,
      router: { exchange: EXCH, getBiasReport: () => ({ exchange: EXCH, updatedAt: NOW, assets: [{ symbol: SYM, direction: 'long' as const, confidence: 85, suggestedPositionPct: 0.1 }], whitelist: [SYM] }), getConfig: () => ({ maxBiasReportAgeHours: 24 }), killSwitch: ks2 },
      indicatorService: { calculateAll: async () => [{ name: 'CompositeMomentum' as const, composite_score: 85, regime_state: 'STRONG_BULLISH' as const, in_cooldown: false, dimension_scores: { hull_big_trend: { value: 1, weight: 1 }, stc_momentum: { value: 1, weight: 1 }, volume_micro: { value: 1, weight: 1 } }, lag_bars: 0, elapsedMs: 0 }] },
      marketData: { exchange: EXCH, snapshotStore: store2, candleStore: candle2, interval: '1m', minimumSeries: 100, seriesLimit: 200 },
    });
    const r = await new PaperFastPathCoordinator(fp2, await svc(d), EXCH).run(SIG, P);
    assert.ok(r.pipelineResult);
  } finally { await fs.rm(d, { recursive: true, force: true }); }
});
