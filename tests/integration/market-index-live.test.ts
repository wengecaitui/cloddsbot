...
test('market index sync hits real Polymarket API', { timeout: 20000 }, async () => {
  ...
  assert.ok(typeof result.byPlatform.polymarket === 'number');
  assert.ok(result.byPlatform.polymarket >= 0);
  if (result.byPlatform.polymarket > 0) {
